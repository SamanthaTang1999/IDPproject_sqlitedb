...
public class AppointmentInfoController {
	...
	public void sendReminder(ActionEvent event) {
		...
		if (appID != null) {
			...
			String ownerName = label_petOwner.getText().substring(13);
			String petName = label_petName.getText().substring(11);
			try {
				Integer ownerID = model.getOwnerID(appID);
				String email = model.getOwnerEmail(ownerID);
				String message = "Dear " + ownerName + "," + newline + newline +
						" Your upcoming vaccination appointment for " + petName
						+ " is on " + datePicker_date.getValue().toString() + " ." + newline +
						"Thank You." + newline + newline
						+ "This mail is autogenerated.";
			
				if (mail.sendMail(email, message)) {
				
				
					log.logFile(null, "info", "Reminder email is sent");
					
					Alert alert = new Alert(AlertType.INFORMATION);
					alert.setTitle("Program says");
					alert.setHeaderText("Reminder email is sent.");
					alert.show();
					...
					Alert alert = new Alert(AlertType.ERROR);
					alert.setTitle("Program says");
					alert.setHeaderText("Reminder email is not sent");
					alert.show();
